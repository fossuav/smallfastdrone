...
const AP_Param::GroupInfo ParametersG2::var_info2[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("THROW_YAW_TYPE", 28, ParametersG2, throw_yaw_type, (float)ModeThrow::ThrowYawType::None),
    ...
    // @Units: deg
    // @User: Advanced
    AP_GROUPINFO("THROW_YAW_DEG", 29, ParametersG2, throw_yaw_deg, 0),
    ...
    // @Description: Enables the navigation source fallback monitor. Requires EK3_SRC_OPTIONS bit 3 (per-core source sets) with EK3_SRC1 configured for GPS and EK3_SRC2 for optical flow, EK3_OPTIONS bit 1 (manual lane switching) and two EKF lanes. On GPS loss or detected GPS spoofing the monitor moves the primary EKF lane to the optical flow lane without triggering the EKF failsafe, and if neither lane can provide position while in Loiter the vehicle changes to AltHold. GPS loss recovers automatically; a spoof detection latches GPS as untrusted until disarm or a pilot source set change. Spoof detection is bounded by how far the two lanes disagree during ordinary flight: it catches a spoofer whose reported velocity is consistent with its position walk, and does not catch a position-only walk slow enough to sit inside that envelope. Set to 2 to also allow arming on the optical flow lane when GPS is unavailable, for example indoors: the GPS lane is taken up once a fix is acquired in flight. Home, RTL and fences are unavailable until that happens unless an origin is set before takeoff.
    // @Values: 0:Disabled, 1:Enabled, 2:Enabled and may arm on the flow lane without GPS
    // @User: Advanced
    ...
    // @Description: Satellites a GPS fix must report before the first fix of a GPS-free arm can be taken up at all, whatever the cross-lane offset says. Off by default because the separation it was added for did not survive a longer look: an indoor fix that sat at 8 or 9 satellites for the first minute of field log 350 climbed to 15-17 over the next, which is open-sky territory, so a count that looks decisive early is only telling you the receiver has not finished acquiring. Set it only where a site has been measured to hold a low count for as long as a flight lasts. It is not a spoof check: a synthesised spoof can report any count it likes.
    // @Range: 0 30
    // @User: Advanced
