...
#33879 pairs with #34251: the latter stops seeding the INS_HNTC2 defaults, the former forces the tuned values into storage so they survive a default change. It is unreviewed, so it sits below the approved three rather than with them.

4. #33879 FFT notch tune: persist the tracking mode and name the notch used - 4 files, comments only, no review
...
6. #34292 AP_OpticalFlow: add FLOW_HGT_MIN, the sensor's minimum focus height - split out of #33484, so it follows it
7. #33568 AP_NavEKF3: fall back to relative aiding when optical flow replaces lost GPS - 4 files, **no review at all**
8. #33569 AP_NavEKF3: make the optical-flow nav gain detune height configurable (EK3_FLOW_GAIN_H) - 3 files, reviewed by peterbarker
9. #33498 AP_NavEKF3: inhibit Z gyro bias from optical flow with no yaw reference - 4 files, comments only, no review, AI-reviewed
