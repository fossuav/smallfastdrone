...
WATCH="FlowGyroZBiasNoYawReference TerrainOffsetGroundEffectRecovery
       AmslAltPreservedOnRearmAtDifferentElevation OpticalFlowFocusHeight
       OpticalFlowAGLKalmanFilter OpticalFlowGPSLossAiding"
# tests that hang or stall rather than fail fast run at the very end, after the
# throw tests: Replay's GPS subtest stops SITL answering and costs ~15 minutes
LAST="Replay"

list_steps() {
  WATCH="$WATCH" LAST="$LAST" python3 - <<'PY'
...
last = os.environ.get('LAST', '').split()
# the 4.7 this branch contains, not the 4.7 tip: tests 4.7 changed after the
# base was cut would otherwise read as ours
base_ref = os.environ.get('SFD_TEST_BASE') or subprocess.run(
    ['git', 'merge-base', 'HEAD', 'upstream/ArduPilot-4.7'], capture_output=True, text=True).stdout.strip()
# tests can be defined on TestSuite and only registered by a vehicle (the MSP
# VTX pair), so the shared file's methods count as the vehicle's own
suite_p = 'Tools/autotest/vehicle_test_suite.py'
suite_cm = meths(open(suite_p, encoding='utf-8', errors='replace').read())
suite_bm = meths(show(base_ref, suite_p))
out = []
for veh, f in (('Copter', 'arducopter.py'), ('Plane', 'arduplane.py'),
    ...
    cm, bm = {**suite_cm, **meths(src)}, {**suite_bm, **meths(base)}
...
summarise() {
  ...
  local p f
  # grep -c already prints 0 and exits 1 when it matches nothing, so an
  # "|| echo 0" fallback appends a second line and every arithmetic test below
  # then dies on "0\n0"
  p=$(grep -c '^>>>> PASSED STEP' "$LOG" 2>/dev/null || true)
  f=$(grep -c '^>>>> FAILED STEP' "$LOG" 2>/dev/null || true)
  echo "log: $LOG"
  echo "passed: $p   failed: $f   crashes: $(grep -c 'Floating point exception' "$LOG" 2>/dev/null || true)"
  if [ "$f" -gt 0 ]; then
    echo "failures:"
    grep -E '^>>>> FAILED STEP' "$LOG" | sed -E 's/^>>>> FAILED STEP: ([^ ]+).*/  \1/'
    # one failure produces both lines below, so label them rather than listing
    # them as if they were separate failures.  The step line names the last
    # exception, which after a failure is often the teardown reboot rather
    # than the test, so the caught one is usually the informative one
    echo "reasons (step):"
    grep -hoE 'FAILED: "[^"]+": .*' "$LOG" | sed 's/^/  /' | cut -c1-160 | sort -u
    echo "reasons (caught in test):"
    grep -hoE 'Exception caught: .*' "$LOG" | sed 's/^/  /' | cut -c1-160 | sort -u
  fi
