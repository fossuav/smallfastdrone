...
backup_name() {
  ...
  # remote backups count too: a backup made and pushed from another clone is
  # not a local branch here, and reusing its index collides on the next push
  for b in $(git for-each-ref --format='%(refname:lstrip=2)' "refs/heads/$stem.*$suffix" 2>/dev/null) \
           $(git for-each-ref --format='%(refname:lstrip=3)' "refs/remotes/*/$stem.*$suffix" 2>/dev/null); do
    i="${b#"$stem."}"; i="${i%"$suffix"}"
