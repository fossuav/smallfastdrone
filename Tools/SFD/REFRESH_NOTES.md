...
Figures for these PRs live in `/mnt/c/support/logs/SFD-O4/pr-plots/`, beside the
logs they were made from, and are regenerated by `prplots.py` kept with them.
They are deliberately outside the repository: GitHub takes images through the web
UI or a follow-up comment, not through `gh`, and committing binaries here would
put them in every clone for no gain. Two rules they were built to, both of which
a reviewer will notice if broken: no second y-axis, ever - split the panel
instead; and one time origin per log, the first message of any type, because
taking it per message type slid an annotation 1.1 s off the event it pointed at.
...
- **Needs its own master PR** `AP_NavEKF: name the per-core source set option`,
  ...
  **Flown on SFD-O4 log14** (`5adc2ea0`): "Using EKF Source Set 2" at 46.5055,
  ...
  Figure for the PR: `pr-plots/srcset_lane_before_after.png` (see below), log11
  against log14, the same aircraft and the same switch action.
- **Needs its own master PR** `AP_NavEKF3: clear the AGL KF velocity when the
  ...
  **not** belong to #33359, #33478 or #33507: those stack on top of the clamp,
  ...
  Figures for the PR: `pr-plots/aglkf_1_replay_ab_log9.png` is the Replay A/B on
  one flight, code before against after, which is the honest before/after because
  the sensor stream is identical; `pr-plots/aglkf_2_ground_windup.png` is the
  real-world pair, log9 running to -7.2 m/s over 88 s on the ground against
  log12 flat at -0.001 over 487 s.
