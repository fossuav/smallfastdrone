...
BRANCH47="${SFD_BRANCH47:-upstream/ArduPilot-4.7}"
...
if [ "$#" -gt 0 ]; then
  ...
  for n in $(prs_all); do
    ...
    mb47="$(git merge-base "$ref" "$BRANCH47" 2>/dev/null)"
    if [ -n "$mb47" ] && git merge-base --is-ancestor "$mb" "$mb47"; then mb="$mb47"; fi
...
for n in $PRS; do
  ...
  ref="$(ref_for "$n")"
  mb="$(git merge-base "$ref" "$MASTER")"
  # A PR raised against the 4.7 branch (#34251) shares only an old master with
  # MASTER, so its net change from there is the whole 4.7 branch history.
  # Measure it from the 4.7 branch instead when that fork point is newer.
  mb47="$(git merge-base "$ref" "$BRANCH47" 2>/dev/null)"
  if [ -n "$mb47" ] && [ "$mb47" != "$mb" ] && git merge-base --is-ancestor "$mb" "$mb47"; then
    mb="$mb47"
  fi
  ...
  cp "$F" "$TMP/cur"
  # keep the pre-merge copy: a stop resolved the wrong way cannot otherwise be
  # undone, because the merge has already folded the clean hunks into the file
  cp "$F" "$APPLIED.before"
