...
```sh
# from a worktree on a new branch off SmallFastDrone-4.7-base, running this
# clone's Tools/SFD by path (the base carries no Tools/SFD); REFRESH_NOTES.md
# "Every refresh" is the full checklist:
...
Tools/SFD/refold_methods.py FILE <previous branch> NAME...
                                    # put back the tests the rebuild wipes
                                    # (REFRESH_NOTES "Local work")
...
Tools/SFD/fix_takeoff_kwargs.py Tools/autotest/arducopter.py
                                    # master's takeoff(altitude_*) -> 4.7's
Tools/SFD/check_suite_load.py --against <previous branch>
                                    # dangling/duplicate registrations
Tools/SFD/check_pr_test_lines.py    # PR test changes a stop resolved away
...
- Throw Mode RPM (#32955) is intentionally excluded until its PR is refreshed.
- REFRESH_NOTES.md opens with the checklist for the next refresh and the known
  state of the shipping branch; REFRESH_HISTORY.md is the dated record.
