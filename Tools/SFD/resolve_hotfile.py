...
def fix_dangling(lines, pr, a_file):
    ...
    try:
        ...
        # a PR raised against the 4.7 branch forks from it later than from master
        b47 = subprocess.run(["git", "merge-base", "refs/sfdpr/%s" % pr, BRANCH47], capture_output=True, text=True).stdout.strip()
        if b47 and b47 != base and subprocess.run(["git", "merge-base", "--is-ancestor", base, b47]).returncode == 0:
            base = b47
