...
bool LogReader::handle_log_format_msg(const struct log_Format &f)
{
	...
	} else if (streq(name, "RISK")) {
	    msgparser[f.type] = NEW_NOTHROW LR_MsgHandler_RISK(formats[f.type]);
    ...
    } else if (streq(name, "ROFM")) {
        msgparser[f.type] = NEW_NOTHROW LR_MsgHandler_ROFM(formats[f.type]);
