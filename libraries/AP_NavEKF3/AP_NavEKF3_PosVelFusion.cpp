...
void NavEKF3_core::SelectVelPosFusion()
{
    ...
    // (a real velZ source, configured and providing data, is always preferred), the
    // AGL KF is valid, not already fusing synthetic zero velocity on the ground, and
    // horizontal speed is low enough that terrain-relative velocity approximates the
...
void NavEKF3_core::FuseVelPosNED()
{
    ...
    if (fuseVelData || fuseVelVertData || fusePosData || fuseHgtData) {
        ...
        if (fusingAglKfVel) {
            ...
            // velocity variance, with only a small numerical floor. The AGL KF
            // already inflates this during RF gaps and fast horizontal flight, so
            // trusting it is what gives the fusion authority even when the main
            // filter is over-confident in velD (small P[6][6]); a large floor here
            // would collapse the Kalman gain and make the fusion ineffective.
