...
void NavEKF3_core::selectHeightForFusion()
{
    ...
    if (extNavDataToFuse && (activeHgtSource == AP_NavEKF_Source::SourceZ::EXTNAV)) {
        ...
        if (dal.get_takeoff_expected() || dal.get_touchdown_expected()) {
            if (is_negative(frontend->_baroGndEffectDeadZone) && !takeOffDetected) {
                // Before takeoff is detected, replace the corrupt baro
                // with a synthetic observation at the pre-ground-effect
                // baro reference.  This anchors PD near zero and
