...
void NavEKF3_core::CovariancePrediction(Vector3F *rotVarVecPtr)
{
    ...
    if (!accelBiasLearningInhibited()) {
        for (uint8_t stateIndex = 13; stateIndex <= 15; stateIndex++) {
            ...
            // In flight with aiding: all axes are observable from velocity/position
            // aiding.
            // In flight without aiding (AID_NONE): the synthetic horizontal
            // observations do not update the bias states (see FuseVelPosNED), so
            // only a near-vertical axis is observable, from the height reference.
            // If XY were left observable their variance would grow unconstrained
            // and the tilt-coupled height innovation would be learned as an XY
            // bias, tilting the level estimate (a baro dip in forward flight).
