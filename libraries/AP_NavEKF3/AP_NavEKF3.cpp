...
const AP_Param::GroupInfo NavEKF3::var_info2[] = {
    ...
    // @Description: EKF optional behaviour. Bit 0 (JammingExpected): Setting JammingExpected will change the EKF behaviour such that if dead reckoning navigation is possible it will require the preflight alignment GPS quality checks controlled by EK3_GPS_CHECK and EK3_CHECK_SCALE to pass before resuming GPS use if GPS lock is lost for more than 2 seconds to prevent bad position estimate. Bit 1 (Manual lane switching): DANGEROUS - If enabled, this disables automatic lane switching. If the active lane becomes unhealthy, no automatic switching will occur. Users must manually set EK3_PRIMARY to change lanes. No health checks will be performed on the selected lane. Use with extreme caution.  Bit 2 (Optflow may use terrain alt): Terrain SRTM data will be used if the vehicle climbs above the rangefinder's range allowing optical flow to be used at higher altitudes. Bit 3 (AGL KF for optflow scaling): Use an IMU-aided AGL Kalman filter (height, vertical velocity and accel-Z bias, fused with rangefinder) to compute the height-above-ground used for optical flow velocity scaling, instead of terrainState-pd. This decouples optical flow scaling from errors in the main filter's vertical position state. Bit 4 (AGL KF velocity for velD): Fuse the 2-state AGL KF vertical velocity as a velocity-down observation in the main filter. Requires a downward range finder and enables the AGL KF automatically. Only used in flight, while no other velocity-down source is delivering data, while the range finder is not the height source, and below the ground speed set by EK3_AGL_VD_SPD. The height source is unchanged. Bit 5 (Optflow assume flat ground): once the terrain offset goes stale, which is normally a climb past the rangefinder's range but is also what a failed rangefinder looks like, keep optical-flow relative position valid instead of triggering an EKF failsafe, using the terrain offset last measured this flight, or terrain database altitude where that is reaching the EKF. Only safe where the ground stays at the height last measured, such as a large indoor floor: an error in that height scales the flow velocity and the filter does not reject it. Needs a height source that is not None and has not timed out. Has no effect where the range finder is itself the height source (EK3_SRC1_POSZ=2), because the terrain offset is not fused while it is, so nothing is ever measured to fall back on. Where terrain data is available it also removes the optical flow altitude limit, as it already does for bit 2.
    // @Bitmask: 0:JammingExpected, 1:ManualLaneSwitching, 2:Optflow may use terrain alt, 3:AGL KF for optflow scaling, 4:AGL KF velocity for velD, 5:Optflow assume flat ground
    // @User: Advanced
...
void NavEKF3::writeOptFlowMeas(const uint8_t rawFlowQuality, const Vector2f &rawFlowRates, const Vector2f &rawGyroRates, const uint32_t msecFlowMeas, const Vector3f &posOffset, float heightOverride, float minHeight)
{
    dal.writeOptFlowMeas(rawFlowQuality, rawFlowRates, rawGyroRates, msecFlowMeas, posOffset, heightOverride, minHeight);

    if (core) {
        for (uint8_t i=0; i<num_cores; i++) {
            core[i].writeOptFlowMeas(rawFlowQuality, rawFlowRates, rawGyroRates, msecFlowMeas, posOffset, heightOverride, minHeight);
        }
    }
}
...
void NavEKF3::writeTerrainData(float alt_m)
{
    ...
    // flat-ground fallback automatically prefers the terrain database where coverage exists
