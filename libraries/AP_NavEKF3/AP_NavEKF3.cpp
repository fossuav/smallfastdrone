...
const AP_Param::GroupInfo NavEKF3::var_info[] = {
    ...
    // @Description: This determines when the filter will use the 3-axis magnetometer fusion model that estimates both earth and body fixed magnetic field states and when it will use a simpler magnetic heading fusion model that does not use magnetic field states. The 3-axis magnetometer fusion is only suitable for use when the external magnetic field environment is stable. EK3_MAG_CAL = 0 uses heading fusion on ground, 3-axis fusion in-flight, and is the default setting for Plane users. EK3_MAG_CAL = 1 uses 3-axis fusion only when manoeuvring. EK3_MAG_CAL = 2 uses heading fusion at all times, is recommended if the external magnetic field is varying and is the default for rovers. EK3_MAG_CAL = 3 uses heading fusion on the ground and 3-axis fusion after the first in-air field and yaw reset has completed, and is the default for copters. EK3_MAG_CAL = 4 uses 3-axis fusion at all times. EK3_MAG_CAL = 7 uses 3-axis fusion on the ground and after the first in-air field and yaw reset has completed. While the vehicle is stationary on the ground the yaw is additionally anchored to the measured magnetic heading, since 3-axis fusion has no yaw observability without motion to separate a yaw error from the body field states. This allows the magnetic field to be learned on the ground before takeoff (useful when swapping batteries with different magnetic signatures) while inhibiting learning during the initial climb when motor magnetic interference is strongest. NOTE : Use of simple heading magnetometer fusion makes vehicle compass calibration and alignment errors harder for the EKF to detect which reduces the sensitivity of the Copter EKF failsafe algorithm. NOTE: The fusion mode can be forced to 2 for specific EKF cores using the EK3_MAG_MASK parameter. NOTE: limited operation without a magnetometer or any other yaw sensor is possible by setting all COMPASS_USE, COMPASS_USE2, COMPASS_USE3, etc parameters to 0 and setting COMPASS_ENABLE to 0. If this is done, the EK3_GSF_RUN and EK3_GSF_USE masks must be set to the same as EK3_IMU_MASK. A yaw angle derived from IMU and GPS velocity data using a Gaussian Sum Filter (GSF) will then be used to align the yaw when flight commences and there is sufficient movement.
...
bool NavEKF3::getLaneDivergenceSigma(uint8_t lane_index, float &vel_sigma_mps) const
{
    if (core == nullptr || lane_index >= num_cores || lane_index == primary) {
        return false;
    }
    const float var = core[lane_index].getVelVarianceNE() + core[primary].getVelVarianceNE();
    if (!is_positive(var)) {
        return false;
    }
    vel_sigma_mps = sqrtf(var);
    return true;
}
...
void NavEKF3::writeTerrainData(float alt_m)
{
    ...
    // forward to the cores when either above-range optical flow option is set, so the
    // flat-ground fallback automatically prefers the terrain database when coverage exists
    if (!option_is_enabled(Option::OptflowMayUseTerrainAlt) &&
        !option_is_enabled(Option::OptflowAssumeFlatGnd)) {
        return;
    }
