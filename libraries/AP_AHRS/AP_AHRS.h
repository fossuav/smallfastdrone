...
public:
    ...
    void writeOptFlowMeas(const uint8_t rawFlowQuality, const Vector2f &rawFlowRates, const Vector2f &rawGyroRates, const uint32_t msecFlowMeas, const Vector3f &posOffset, const float heightOverride, const float minHeight = 0.0f);
    ...
    // The configured backend decides whether the reset happens; the others
    // follow it, or go ahead anyway when it has no datum of its own
    // Returns true if any backend performed the reset
